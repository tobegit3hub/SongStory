...
public class StoryDatabaseAccessor {
	...
	public static void init(Context context) {
		if (StoryDatabaseAccessor.context != null) {
			throw new IllegalArgumentException(
					"can not init the dbaccessor twice");
		}
		StoryDatabaseAccessor.context = context;
		databaseHelper = new StoryDatabaseHelper(context);
	}

	public static ArrayList<SongStory> getSongStories(int categoryNumber)
			throws JSONException {
		ArrayList<SongStory> stories = new ArrayList<SongStory>();
		SQLiteDatabase db = databaseHelper.getWritableDatabase();
		Cursor c = null;
		try {
			c = db.query(StoryDatabaseHelper.TABLE_SONG_STORY,
					new String[] { "def" }, " category_number = ?",
					new String[] { String.valueOf(categoryNumber) }, null,
					null, null);
	...
	public static SongStory getSongStory(int categoryNumber, int songNumber)
			throws JSONException {
		...
		try {
			c = db.query(
					StoryDatabaseHelper.TABLE_SONG_STORY,
					new String[] { "def" },
					" category_number = ? and song_number = ?",
					new String[] { String.valueOf(categoryNumber),
							String.valueOf(songNumber) }, null, null, null);
	...
	// db.execSQL("delete from " + TopicDatabaseHelper.TABLE_CATEGORY +
	// " where category_id = ?"
	// + categoryId);
