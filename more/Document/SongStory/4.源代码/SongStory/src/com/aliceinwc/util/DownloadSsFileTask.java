...
public class DownloadSsFileTask extends AsyncTask<Integer, Integer, Boolean> {
	...
	protected void onPreExecute() {
		...
		notification.contentView.setProgressBar(R.id.task_progress, 100, 0,
				false);
	}
	...
	protected Boolean doInBackground(Integer... params) {
		...
		String requestUrl = NetworkUtil.BASE_URL
				+ "response.php?category_number=" + categoryNumber
		...
		try {
			...
			if (jsonObject.getBoolean("is_exist")) {
				...
				String downloadFileName = "" + categoryNumber + song_number
						+ ".ss";
				if (song_number <= 9) {
					downloadFileName = "" + categoryNumber + "0" + song_number
							+ ".ss";
				}
				OutputStream output = new FileOutputStream(FileUtil.SS_PATH
						+ downloadFileName);
				...
				notification.contentView.setProgressBar(R.id.task_progress,
						100, progress, false);
				notificationManager.notify(0, notification);
