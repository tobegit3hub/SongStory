...
public class StoryDetailActivity extends Activity implements OnTouchListener {
	...
	private void textInit() {
		...
		float lyricsTextSize = getResources().getDimension(R.dimen.lyrics_text_size);
//		Toast.makeText(activity, ""+lyricsTextSize/2, Toast.LENGTH_SHORT).show();
	...
	private void buttonInit() {
		...
		mPlayPrevious.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					SongStory newSongStory = StoryDatabaseAccessor.getSongStory(songStory.categoryNumber, songStory.songNumber-1);
					if(newSongStory == null){
						Toast.makeText(activity, R.string.most_previous_song, Toast.LENGTH_SHORT).show();
					}else{
						Toast.makeText(activity, R.string.previous_song, Toast.LENGTH_SHORT).show();
		...
		mPlayNext.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				try {
					SongStory newSongStory = StoryDatabaseAccessor.getSongStory(songStory.categoryNumber, songStory.songNumber+1);
					if(newSongStory == null){
						Toast.makeText(activity, R.string.most_next_song, Toast.LENGTH_SHORT).show();
					}else{
						Toast.makeText(activity, R.string.next_song, Toast.LENGTH_SHORT).show();
						songStory = newSongStory;
						isContinue = false;
	...
	private void menuButtonInit() {
		...
		mHome.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(crackTime==49){
					Toast.makeText(activity, "亲，你已经点了七七四十九次了T_T", Toast.LENGTH_LONG).show();	
				}
				...
				if(crackTime%8==7){
					...
					if(isCrackMode){ // back to normal
						...
						imageView.setBackgroundResource(R.drawable.back_normal_mode);
						
					}else{ // let's crack
						isCrackMode = true;
						
						imageView.setBackgroundResource(R.drawable.in_crack_mode);
					}
					

					SharedPreferences.Editor editor = settings.edit();
					editor.putBoolean(WelcomeActivity.IS_CRACK_MODE, isCrackMode);
					editor.commit();
